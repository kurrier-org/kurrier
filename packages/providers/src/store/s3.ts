import {
    ...
    HeadBucketCommand,
    CreateBucketCommand,
    PutPublicAccessBlockCommand, ListObjectsV2Command,
    DeleteObjectCommand,
...
import { z } from "zod";
import type {
    AddBucketResult, AddFolderResult, DeleteEntryResult,
    DownloadResult,
    ListPathEntry,
    ListPathResult,
    StorageProvider, UploadUrlResult,
    VerifyResult
} from "../core";
...
export class S3Store implements StorageProvider {
    ...
    async addBucket(
        ...
        let bucket = "";
        try {
            bucket = BucketNameSchema.parse(String(input.bucket ?? "").trim().toLowerCase());
    ...
    async deleteEntry(
        ...
        const bucket = BucketNameSchema.parse(String(input.bucket ?? "").trim().toLowerCase());
        ...
        try {
            if (type === "file") {
                ...
                if (!key) return { ok: false, message: "Invalid file path", meta: { providerId: id, bucket, path: uiPath } };

                await this.s3.send(new DeleteObjectCommand({ Bucket: bucket, Key: key }));
                return { ok: true, message: "Deleted", meta: { providerId: id, bucket, key } };
            }

            const prefix = this.folderUiPathToPrefix(uiPath);
            if (!prefix) return { ok: false, message: "Invalid folder path", meta: { providerId: id, bucket, path: uiPath } };
    ...
    async downloadUrl(
        ...
        const uiPath = String(input.path ?? "");
        const key = this.uiPathToKey(uiPath);
        const expiresIn = Math.max(10, Math.min(Number(input.expiresIn ?? 60), 3600));

        if (!key) {
            return { ok: false, message: "Invalid file path", meta: { providerId: id, bucket, path: uiPath } };
        }
        ...
        try {
            const url = await getSignedUrl(
                this.s3,
                new GetObjectCommand({ Bucket: bucket, Key: key, ResponseContentDisposition: contentDisposition }),
                { expiresIn },
            );
    ...
    async uploadUrl(
        ...
        const contentType = input.contentType ? String(input.contentType) : "application/octet-stream";
        const cacheControl = input.cacheControl ? String(input.cacheControl) : undefined;
    ...
    private buildDefaultCorsOrigins() {
        ...
        const webUrl = process.env.WEB_URL ? String(process.env.WEB_URL).trim() : "";
        if (webUrl) {
            try {
    ...
    private getBucketRegionFromError(e: any) {
        return e?.$response?.headers?.["x-amz-bucket-region"] || e?.BucketRegion || null;
    }
    ...
    private async headBucketStrict(bucket: string): Promise<HeadBucketStrictResult> {
        try {
            await this.s3.send(new HeadBucketCommand({ Bucket: bucket }));
            return { exists: true };
        } catch (e: any) {
