...
// 		tx.select().from(labels).where(sql`${labels.scope} = ${selectedScope}`).orderBy(asc(labels.name)),
// 	);
...
// 		tx.select().from(labels).where(sql`${labels.scope} = 'thread'`).orderBy(asc(labels.name)),
// 	);
//
...
//
//
// export const fetchContactLabelsByContactIds = async (
//     contactIds: string[],
// ) => {
