...
import {FetchIdentityMailboxListResult, moveToFolder} from "@/lib/actions/mailbox";
import {toast} from "sonner";

function MoveToFolder({ identityMailboxes, activeMailbox }: { identityMailboxes: FetchIdentityMailboxListResult, activeMailbox: MailboxEntity; }) {
    const [opened, { open, close }] = useDisclosure(false);
    const params = useParams();

    const entry =
        identityMailboxes.find(im => im.identity.publicId === params.identityPublicId) ?? null;
            ...
            <Modal opened={opened} onClose={close} title="Move to">
                <div className="space-y-4">
                    ...
                    <div className="flex gap-2 justify-end">
                        ...
                        <Button onClick={onConfirm} loading={submitting} disabled={!destId || destId === "none"}>
                            Move
                        </Button>
                    </div>
                </div>
