...
import { CalendarClock, Check, ChevronDown, ChevronUp, SendHorizonal, X } from "lucide-react";
...
function ScheduleSend() {
    ...
    return (
        <>
            <Modal centered opened={pickerOpened}
                   onClose={closePicker}
            ...
            <Modal centered opened={opened} closeOnClickOutside={false} onClose={close} title={<span className={"text-xl"}>Schedule Send</span>} size="sm" zIndex={1001}>
                <div className={"my-2 p-2 font-semibold"}>{tzName?.alternativeName} ({tzName?.abbreviation})</div>
                {presets.map(preset => {
                    return <button
                        key={preset.label}
                        className={"w-full px-2 text-left rounded hover:bg-gray-100 flex gap-4 justify-between dark:hover:bg-neutral-700"}
                        onClick={() => {
                ...
                <Button leftSection={<CalendarClock size={16} />} variant={"light"} fullWidth={true} onClick={() => {
                    close()
                    openPicker()
                }}>
                    Pick date and time
            ...
            >
                <Menu.Target>
                    <Button type="button" size="sm" className="!px-2 !rounded-l-xs !rounded-r-4xl">
            <span className="inline-flex items-center gap-1.5">
              {scheduled ? <CalendarClock size={14} /> : null}

                {!scheduled ? (
                    <>
                        <ChevronDown size={14} className="group-data-[expanded=true]:hidden" />
                        <ChevronUp size={14} className="hidden group-data-[expanded=true]:block" />
                    </>
                ) : (
                    ...
                    {!scheduled ? (
                        ...
                        <>
                            <Menu.Item
                                ...
                                rightSection={<span className="text-[11px] text-neutral-500">{formatWhen(scheduledAt!)}</span>}
                                onClick={() => {}}
                            >
                                Scheduled
                            </Menu.Item>
