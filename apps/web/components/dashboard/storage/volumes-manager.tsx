...
function VolumeStatusPill({ verified }: { verified: boolean }) {
    return (
      ...
      {verified ? <CheckCircle className="size-3.5" /> : <Clock className="size-3.5" />}
            {verified ? "Provider verified" : "Provider not verified"}
    </span>
    );
}

export default function VolumesManager({ userProviders, volumes, providerSelectOptions }: { userProviders: SyncProvidersRow[]; volumes: DriveVolumeEntity[], providerSelectOptions: { label: string; value: string }[]; }) {
    ...
    const openAddVolumeForm = async () => {
        const openModalId = modals.open({
            ...
            children: (
                <div className="p-2">
                    <AddVolumeForm userProviders={userProviders}
                    ...
                    <div className="space-y-3">
                        ...
                        {volumes.length === 0 ? (
                            ...
                            <div className="grid gap-4 sm:grid-cols-1 lg:grid-cols-2">
                                {volumes.map((v) => {
                                    const volumeProvider = userProviders.find((provider) => provider.id === v.providerId);
                                    const verification = userProviders.find((provider) => provider.id === v.providerId)?.metaData?.verification;
                                            ...
                                            <div className="flex items-start gap-2">
                                                ...
                                                <div className="min-w-0">
                                                    <div className="truncate font-semibold text-brand-foreground flex gap-2 flex-wrap items-center">
                                                        <span>{v.label}</span>
                                                        <span className="text-xxs text-muted-foreground font-normal">· {providerType?.toUpperCase()}</span>
                                                    </div>

                                                    <div className="mt-2 flex flex-wrap items-center gap-2">
                                                        <VolumeStatusPill verified={isLocal ? true : verification?.store} />
                                                        {v.createdAt ? (
                                                            <span className="text-xs text-muted-foreground">
                                Created: {dayjs(v.createdAt).format("MMM D, YYYY")}
                              </span>
                                                        ) : null}
                                                    </div>
                                                </div>
                                        ...
                                        <div className="flex gap-2 sm:gap-3 w-full sm:w-auto flex-wrap">
                                            <Button
                                                ...
                                                href={v.code === "home" ? "/dashboard/drive" : `/dashboard/drive/volumes/${v.publicId}`}
                                                component={Link}
                                            >
                                                View
                                            </Button>
