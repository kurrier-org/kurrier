...
export default function NewContactForm({
    ...
    createContactAction: (_prev: FormState, formData: FormData) => Promise<FormState>;
    user: User | null;
    publicConfig: PublicConfig
}) {
    const [preview, setPreview] = React.useState<string | null>(profilePictureUrl ?? null);
    ...
    const newContactPublicId = React.useRef(
        contact?.publicId ?? nanoid(10)
    );
    ...
    const fields: FieldConfig[] = [
        {
            el: (
                <div className="flex flex-col items-center mb-4">
                    ...
                    {contact?.profilePicture ? <input type="hidden" name="profilePicture" defaultValue={contact?.profilePicture || ""}  /> : <input type="hidden" name="profilePicture" />}
                    {contact?.profilePictureXs ? <input type="hidden" name="profilePictureXs" defaultValue={contact?.profilePictureXs || ""}  /> : <input type="hidden" name="profilePictureXs" />}
                </div>
            ),
    ...
    return (
        <Container variant="narrow" className="my-8">
            <div className="flex items-center justify-between my-2 mb-8">
                <h1 className="text-xl font-bold text-foreground">{contact ? `Edit ${contact.firstName} ${contact.lastName}` : "New Contact Form"}</h1>
            </div>
            <ReusableForm fields={fields} action={createContactAction} onSuccess={(newContact) => router.push(`/dashboard/contacts/${newContact.publicId}`)} />
        </Container>
    );
}
