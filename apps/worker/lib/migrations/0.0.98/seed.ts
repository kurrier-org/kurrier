...
const seedDefaultCalendar = async (userId: string) => {
    const [existing] = await db
        ...
        .from(calendars)
        .where(
            and(eq(calendars.ownerId, userId), eq(calendars.isDefault, true)),
        )
        .limit(1);
    ...
    const [davCalendar] = await davDb.select().from(davCalendars).where(eq(davCalendars.id, Number(davCalendarInstance.calendarid)))

    if (!davCalendar) {
        console.error(`User ${userId}: cannot create default calendar — no CalDAV calendar found`);
        return null;
    }

    const syncToken =
        davCalendar.synctoken != null
            ? String(davCalendar.synctoken)
            : "1";
