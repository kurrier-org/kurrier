...
const stripQueryHash = (s: string) => (s.split("?")[0] ?? "").split("#")[0] ?? s;
...
const toHomeRelativePath = (ownerId: string, href: string) => {
    ...
    if (!clean.startsWith(prefix)) throw new Error("deletePath: href outside home");
...
export const deletePath = async (opts: {
    ...
    const [vol] = await db
        .select()
        .from(driveVolumes)
        .where(and(eq(driveVolumes.id, volumeId), eq(driveVolumes.ownerId, ownerId)))
        .limit(1);
    ...
    if (!davRes.ok && davRes.status !== 404) {
        ...
        throw new Error(`WebDAV DELETE failed: ${davRes.status} ${davRes.statusText}\n${body}`);
    }
