...
export const addFolderPath = async (opts: {
    ...
    const parentDavPath = ensureTrailingSlash(within.length ? joinUrlPath(davBase, within.join("/")) : davBase);
    const folderDavPath = ensureTrailingSlash(joinUrlPath(parentDavPath, folderName));
    ...
    if (!(res.status === 201 || res.status === 405)) {
        ...
        throw new Error(`WebDAV MKCOL ${folderDavPath} failed: ${res.status} ${res.statusText}\n${body}`);
    }
