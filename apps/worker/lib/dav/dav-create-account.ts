...
const temporaryDelete = async (userId: string): Promise<void> => {
	...
	await davDb.delete(davPrincipals).where(eq(davPrincipals.uri, principalUri));

	await davDb.delete(davUsers).where(eq(davUsers.username, davUsername));

	console.log(`temporaryDelete: fully wiped DAV + DB state for user ${userId}`);
};
...
export async function ensureDefaultAddressBook(ctx: DavUserContext) {
	...
	if (!existingAddressbook) {
		...
		console.info(
			`[DAV] Created addressbook for ${davUsername} status=${res.status}`,
		);
	}
...
export async function ensureDefaultCalendar(ctx: DavUserContext) {
	...
	if (!existingCalInstance) {
		...
		console.info(
			`[DAV] Created calendar for ${davUsername} status=${res.status}`,
		);
	}
...
export async function buildContextFromExistingAccount(
	userId: string,
): Promise<DavUserContext | null> {
	const [davAccount] = await db
		.select()
		.from(davAccounts)
		.where(eq(davAccounts.ownerId, userId))
