...
import {
    ...
    CalendarEventAttendeeInsertSchema, contacts,
} from "@db";
...
export function normaliseItipRequestToVevent(itip: string): string {
    ...
    const incomingProdid = vcal.getFirstPropertyValue("prodid") as string | undefined;
    cleanVcal.addPropertyWithValue("prodid", incomingProdid || "-//Kurrier//Calendar//EN");
...
async function handleIncomingRequest({
    ...
    if (!mailbox?.identityId) {
        console.warn("handleIncomingRequest: mailbox has no identity", { mailboxId });
        return;
    }
    ...
    const [calendar] = await db
        ...
        .where(
            and(
                eq(calendars.ownerId, msg.ownerId),
...
async function applyIncomingRequest({
    ...
    if (!existing) {
        ...
        await davWorkerQueue.add("dav:calendar:create-event", { eventId, notifyAttendees: false });
    } else {
        eventId = existing.id;
        ...
        await davWorkerQueue.add("dav:calendar:update-event", { eventId, notifyAttendees: false });
    }
