...
export function buildICalEvent(
	...
	if (eventRow.rawIcs) {
		try {
			...
			vevent =
				vcal.getFirstSubcomponent("vevent") ?? new ICAL.Component("vevent");
	...
	const organizerGuest = attendees.find((g) => g.isOrganizer) ?? null;
	...
	for (const g of attendees) {
		...
		const wantsRsvp = shouldDefaultRsvp ? g.rsvp !== false : g.rsvp === true;
