...
const syncBook = async (book: AddressBookEntity, defaultDavBookId: number | null) => {
    ...
    if (!davBookId) {
        console.info(
            "[DAV SYNC] Skipping book without davAddressBookId",
            book.id,
        );
        return;
    }
    ...
    if (deletedIds.length) {
        console.info(
            "[DAV SYNC] Deleted local contacts removed remotely:",
            {
                bookId: book.id,
