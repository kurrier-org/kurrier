import {
    db,
    identities,
    mailboxes, mailboxThreads, messages
} from "@db";
...
function attachRealtimeEventHandlers(identityId: string, client: ImapFlow, imapInstances: Map<string, ImapFlow>) {
    ...
    client.on("flags", async (ev: FlagsEvent) => {
        ...
        if (!uid) {
            console.warn(`[realtime:${identityId}] could not resolve UID for seq=${ev.seq}`);
            return;
        }
...
async function startRealtimeSyncForIdentity(identityId: string, client: ImapFlow, imapInstances: Map<string, ImapFlow>) {
    try {
        await client.getMailboxLock("INBOX");
        attachRealtimeEventHandlers(identityId, client, imapInstances);
        idleForever(identityId, client);
    } catch (err) {
        console.error(`[realtime:${identityId}] failed to start realtime sync`, err);
    }
}
...
export async function stopRealtimeForIdentity(
    ...
    if (cmdClient) {
        try { await cmdClient.logout(); } catch {}
        imapInstances.delete(identityId);
    }

    console.log(`[kurrier] Stopped realtime + command IMAP clients for identity ${identityId}`);
}
...
export const imapIdleSync = async (
    ...
    for (const identity of identityRows) {
        await startRealtimeForIdentity(identity.id, idleImapInstances, imapInstances);
    }
};
