...
export async function addNewFolder(
    ...
    if (data.parentId) {
        ...
        if (parent?.metaData?.imap) {
            ...
            delimiter =
                String(parent.metaData.imap.delimiter ?? (await detectDelimiter(client)));
        } else {
    ...
    const newPath = parentPath ? `${parentPath}${delimiter}${safeName}` : safeName;
    ...
    const [row] = await db
        ...
        .values({
            ...
            parentId: String(data.parentId)?.trim().length > 0 ? String(data.parentId)  : null,
    ...
    console.log(`✅ Created mailbox "${safeName}" (${newPath}) for ${data.identityId}`);
    return row;
}
