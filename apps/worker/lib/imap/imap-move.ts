...
export const moveMail = async (
    ...
    const { threadId, mailboxId: fromMailboxId, op, toMailboxId, moveImap } = data;
    ...
    if (!destMailboxRow) {
        console.warn(`[mail:move] No destination for op=${op} identity=${srcMailbox.identityId}`);
        return;
    }
    ...
    const threadMsgs = await db
        ...
        .from(messages)
        .where(and(eq(messages.threadId, threadId), eq(messages.mailboxId, fromMailboxId)));
    ...
    if (moveImap) {
        ...
        if (!byPath.size) {
            console.debug("[mail:move] No IMAP-backed UIDs to move; DB-only move will proceed.");
        } else {
    ...
    await db.transaction(async (tx) => {
      ...
      `;
        ...
        await tx
            ...
            .where(and(eq(mailboxThreads.threadId, threadId), eq(mailboxThreads.mailboxId, fromMailboxId)));
    });
};
