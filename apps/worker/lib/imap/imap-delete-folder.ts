...
export async function deleteFolder(args: DeleteFolderArgs, client: ImapFlow) {
    ...
    const [mbx] = await db.select().from(mailboxes)
        .where(and(eq(mailboxes.id, args.mailboxId), eq(mailboxes.identityId, args.identityId)))
        .limit(1);
    ...
    const children = await db.select({ id: mailboxes.id }).from(mailboxes)
        .where(and(eq(mailboxes.identityId, args.identityId), eq(mailboxes.parentId, mbx.id)));
    if (children.length) throw new Error("Folder has subfolders. Delete them first.");

    try {
        await client.mailboxDelete(path);
    } catch (err: any) {
        // treat "not found" as success
        if (!/not found|no such|does not exist/i.test(String(err?.message))) throw err;
    }
