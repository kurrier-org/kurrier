...
import {apiSuccess, validateApiKey, validateJSONBody} from "../../../../../lib/api-helpers";
...
export default defineEventHandler(async (event) => {
    ...
    const [identity] = await db.select().from(identities).where(eq(identities.id, data.identityId));
    if (!identity) {
        throw createError({
            statusCode: 400,
    ...
    const [sentMailbox] = await db.select().from(mailboxes).where(and(
